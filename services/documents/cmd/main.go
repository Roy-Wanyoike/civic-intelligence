...
// Config holds the documents service's runtime configuration.
type Config struct {
	HTTPAddr        string        `env:"HTTP_ADDR" default:":8083"`
	...
	ChunkTarget     int           `env:"CHUNK_TARGET_TOKENS" default:"512"`
	ChunkOverlap    int           `env:"CHUNK_OVERLAP_TOKENS" default:"128"`
...
func main() {
	...
	log := observability.NewLogger(os.Stderr, cfg.LogLevel, "documents")
	log.Info("starting documents service", observability.String("addr", cfg.HTTPAddr))
	...
	log.Info("documents service stopped")
}
