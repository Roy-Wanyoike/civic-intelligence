...
func TestRunService_Run_RejectsNonReadyScenario(t *testing.T) {
        ...
        // Create a DRAFT scenario (not READY) and seed it into the SAME repo
        // that rs uses, so Run() actually finds it. Using a different repo (as a
        // previous version of this test did) caused Run() to fail with a
        // "scenario not found" error and the test asserted nothing.
        sc := golden.SimpleDeterministicScenario
        sc.ID = "scn-draft-test"
        sc.Status = domain.ScenarioStatusDraft
        sc.TenantID = "tenant-golden"
        if err := rs.Repos.Create(ctx, sc); err != nil {
                t.Fatalf("seed draft scenario: %v", err)
        }

        _, err := rs.Run(ctx, "tenant-golden", "scn-draft-test", "user-test", 1)
        if err == nil {
                t.Fatal("expected error when running a non-READY scenario; got nil")
        }
