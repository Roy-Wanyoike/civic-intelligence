...
// ValidateConstraints checks scenario constraints. Phase 18 §21.
//
// It performs two kinds of check:
//
//  1. Numeric range check — every numeric variable's Value must lie within
//     its declared [Minimum, Maximum] window. This catches gross out-of-range
//     inputs even when no explicit constraint Expression is declared.
//  2. Constraint Expression evaluation — every ScenarioConstraint with a
//     non-empty Expression is evaluated against the scenario variables via
//     EvaluateConstraint (issue #210). The Expression can reference any
//     scenario variable by name and supports comparison + logical operators,
//     e.g. "tax_rate <= 0.30 && implementation_delay < 6". An Expression that
//     evaluates to false (or fails to parse) is a validation failure.
func ValidateConstraints(s Scenario) error {
        ...
        for i, c := range s.Constraints {
                if err := EvaluateConstraint(c.Expression, s.Variables); err != nil {
                        return fmt.Errorf("constraint[%d] %s: %w", i, c.ID, err)
                }
        }
        return nil
}
