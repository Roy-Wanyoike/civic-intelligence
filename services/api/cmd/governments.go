...
//      GET /api/v1/constitution                 -- get the constitution metadata + chapters
//      GET /api/v1/constitution/articles        -- list constitution chapters + articles
//      GET /api/v1/constitution/articles/{id}  -- get a single constitution article
//      GET /api/v1/transitions                  -- list government transitions
//
// The following endpoint is NOT registered, even though earlier versions of
// this file documented it:
//
...
// makeConstitutionArticlesHandler returns the list handler for
// GET /api/v1/constitution/articles. The response carries the curated
// chapter + article tree (issue #212), tagged with the FACT reality layer
// — the platform never reinterprets constitutional text. The chapters
// and articles are sourced from the kenya_seed package; the frontend
// keeps its hardcoded data file as a fallback when the API is
// unreachable.
func makeConstitutionArticlesHandler() http.HandlerFunc {
        return func(w http.ResponseWriter, r *http.Request) {
                if r.Method != http.MethodGet {
                        writeError(w, http.StatusMethodNotAllowed, "method_not_allowed", "method not allowed")
                        return
                ...
                w.Header().Set("Content-Type", "application/json")
                chapters := governmentData.constitution.Chapters
                if chapters == nil {
                        chapters = kenya_seed.KenyaConstitutionChapters
                }
                writeJSON(w, 200, map[string]any{
                        "chapters":      chapters,
                        "count":         len(chapters),
...
// makeConstitutionArticleDetailHandler returns the handler for
// GET /api/v1/constitution/articles/{id}. The {id} path parameter is the
// article's stable ID (e.g. "article-1", "article-10"). Responds 404 if the
// article ID is not in the curated set.
func makeConstitutionArticleDetailHandler() http.HandlerFunc {
        return func(w http.ResponseWriter, r *http.Request) {
                if r.Method != http.MethodGet {
                        writeError(w, http.StatusMethodNotAllowed, "method_not_allowed", "method not allowed")
                        return
                ...
                w.Header().Set("Content-Type", "application/json")
                id := strings.TrimPrefix(r.URL.Path, "/api/v1/constitution/articles/")
                article, ok := kenya_seed.FindKenyaConstitutionArticle(government.ID(id))
                if !ok {
                        writeError(w, http.StatusNotFound, "not_found", "constitution article not found")
                        return
                }
                writeJSON(w, 200, map[string]any{
                        "article":        article,
