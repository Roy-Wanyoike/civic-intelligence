...
func main() {
	...
	userEmailStore := NewUserEmailStore()
        ...
        // no-slash form to the with-slash form (issue #266). The handlers
        // normalise the path before extracting the tail (issue #264).
...
// BillsAdapter is the subset of the Kenya Law adapter that the bills +
// trending handlers depend on. Declared as an interface so tests can
// substitute a mock (e.g. one that returns an error) without spinning
// up a real HTTP server. *kenya_law.Adapter satisfies this interface
// implicitly.
type BillsAdapter interface {
        // DiscoverBills crawls the upstream source and returns the current
        // list of Bill candidates. Implementations MUST return a non-nil
        // error when the upstream is unreachable so the handler can fall
        // back to kenya_seed.SampleBills (issue #265).
        DiscoverBills(ctx context.Context) ([]kenya_law.BillCandidate, error)
        // FetchBill downloads the HTML for a single Bill detail page.
...
func makeBillsHandler(adapter BillsAdapter) http.HandlerFunc {
        return func(w http.ResponseWriter, r *http.Request) {
                ...
                // Issue #265: when the live crawl of new.kenyalaw.org fails
                // (network error, 403, timeout, sandbox IP block), fall back to
                // the seed Bills rather than 503. Operators see the upstream
                // failure in the logs; clients get a 200 with `degraded: true`
                // + `source: "seed"` so the API contract stays intact.
                bills, err := adapter.DiscoverBills(ctx)
                source := "live"
                degraded := false
                if err != nil {
                        log.Printf("bills handler: live crawl failed, falling back to seed data: %v", err)
...
func makeBillDetailHandler(adapter BillsAdapter, aiServiceURL string) http.HandlerFunc {
        return func(w http.ResponseWriter, r *http.Request) {
                ctx, cancel := context.WithTimeout(r.Context(), 30*time.Second)
                defer cancel()
                ...
                // the bill URL by re-discovering (in production, this would be a
                // DB lookup). Issue #265: when the live crawl fails, fall back
                // to the seed Bills and look the ID up there — still returning
                // 200 with `source: "seed"` + `degraded: true` so the API
                // contract stays intact for sandboxed CI + upstream outages.
                bills, err := adapter.DiscoverBills(ctx)
                source := "live"
                degraded := false
                if err != nil {
                        log.Printf("bill detail handler: live crawl failed, falling back to seed data: %v", err)
                        source = "seed"
                        degraded = true
                        // Try the seed slice directly — avoids a second failed crawl.
                        if found := kenya_seed.FindSampleBillByID(billID); found != nil {
                                year := 0
...
func findBillByID(ctx context.Context, adapter BillsAdapter, billID string) (*kenya_law.BillCandidate, error) {
        if billID == "" {
...
func handleBillTimeline(w http.ResponseWriter, r *http.Request, adapter BillsAdapter, billID string) {
        ctx, cancel := context.WithTimeout(r.Context(), 30*time.Second)
        defer cancel()
...
func handleBillChanges(w http.ResponseWriter, r *http.Request, adapter BillsAdapter, billID string) {
        ctx, cancel := context.WithTimeout(r.Context(), 30*time.Second)
        defer cancel()
...
func handleBillSummary(w http.ResponseWriter, r *http.Request, adapter BillsAdapter, billID, aiServiceURL string) {
        if billID == "" {
                writeError(w, http.StatusBadRequest, "bad_request", "bill ID required")
                return
        }
...
func makeTrendingHandler(adapter BillsAdapter) http.HandlerFunc {
        return func(w http.ResponseWriter, r *http.Request) {
                ctx, cancel := context.WithTimeout(r.Context(), 30*time.Second)
                defer cancel()

                // Issue #265: when the live crawl of new.kenyalaw.org fails,
                // fall back to the seed Bills rather than 503 so the trending
                // endpoint keeps its 200 contract (with `degraded: true` +
                // `source: "seed"`) for sandboxed CI + upstream outages.
                bills, err := adapter.DiscoverBills(ctx)
                source := "live"
                degraded := false
                if err != nil {
                        log.Printf("trending handler: live crawl failed, falling back to seed data: %v", err)
                        bills = kenya_seed.SampleBills
                        source = "seed"
                        degraded = true
                }
