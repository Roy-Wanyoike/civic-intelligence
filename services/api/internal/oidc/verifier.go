...
func (v *KeycloakVerifier) parseAndVerify(ctx context.Context, tokenStr string) (auth.Claims, error) {
	...
	// Verify the signature + unmarshal claims in one shot. go-jose also
	// enforces that the signature was produced by the supplied key.
	var jc joseClaims
	if err := tok.Claims(key, &jc); err != nil {
