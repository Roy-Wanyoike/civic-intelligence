// Package parliament — HTML parser for the Parliament of Ethiopia Bills
// listing page (parliament.gov.et/bills).
//
// The Parliament of Ethiopia publishes Bills using a card-per-bill layout that
// looks (schematically) like:
//
//	<div class="bill-card">
//	  <h3 class="bill-title"><a href=".../hate-speech-proclamation.pdf">Proclamation on Hate Speech and Disinformation</a></h3>
//	  <div class="bill-meta">
//	    <span class="bill-number">Proclamation No. 1234/2024</span>
//	    <span class="bill-sponsor">Minister of Peace</span>
//	    <span class="bill-stage">First Reading</span>
//	    <span class="bill-date">12 March 2024</span>
//	    <span class="bill-house">House of Peoples' Representatives</span>
//	  </div>
//	</div>
//
...
// stageMapping maps the lowercased substrings the Parliament of Ethiopia uses
// on its Bills pages to the canonical stage codes from
// internal/ethiopia_data.go. Order matters: more specific patterns MUST come
// first.
var stageMapping = []struct {
	...
	{"house of federation review", "FEDERATION_REVIEW"},
	{"federation review", "FEDERATION_REVIEW"},
	{"house of federation", "FEDERATION_REVIEW"},
...
// MapStageText maps a raw stage string published by parliament.gov.et to the
// canonical Ethiopia stage code. Returns "" when no mapping is known.
...
// ParseBillsListing parses the HTML of a parliament.gov.et Bills listing page
...
// ParseBillDetail extracts Bill metadata from a parliament.gov.et Bill detail
...
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	...
	if len(bills) == 0 {
		...
		if title == "" {
			return nil, fmt.Errorf("ethiopia.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "ethiopia.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		...
		Sponsor:       b.Sponsor,
		PublishedAt:   parseEthiopiaDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "ethiopia.parliament.HTMLParser",
...
// parseEthiopiaDate tries the common date layouts used by parliament.gov.et.
// Returns the zero time if nothing parseable was provided.
func parseEthiopiaDate(s string) time.Time {
