// Package parliament — HTML parser for the Parliament of Morocco Bills
// listing page (parlement.ma/fr/projets-lois).
//
// The Parliament of Morocco publishes Bills using a card-per-bill layout that
// looks (schematically) like:
//
//	<div class="bill-card">
//	  <h3 class="bill-title"><a href=".../loi-donnees-personnelles.pdf">Loi sur la protection des données personnelles</a></h3>
//	  <div class="bill-meta">
//	    <span class="bill-number">Projet de Loi N° 12.24</span>
//	    <span class="bill-sponsor">Ministre de la Justice</span>
//	    <span class="bill-stage">Première Lecture</span>
//	    <span class="bill-date">12 Mars 2024</span>
//	    <span class="bill-house">House of Representatives</span>
//	  </div>
//	</div>
//
...
// stageMapping maps the lowercased substrings the Parliament of Morocco uses on
// its Bills pages to the canonical stage codes from internal/morocco_data.go.
...
var stageMapping = []struct {
	...
	{"chambre des conseillers", "COUNCILLORS_REVIEW"},
	{"councillors review", "COUNCILLORS_REVIEW"},
	{"house of councillors", "COUNCILLORS_REVIEW"},
	{"final vote", "FINAL_VOTE"},
	{"vote final", "FINAL_VOTE"},
	{"adoption", "FINAL_VOTE"},
	{"promulgation royale", "ROYAL_PROMULGATION"},
	{"royal promulgation", "ROYAL_PROMULGATION"},
	{"promulgation", "ROYAL_PROMULGATION"},
	{"promulgué", "ROYAL_PROMULGATION"},
	{"rejet", "REJECTED"},
	{"retiré", "WITHDRAWN"},
}

// MapStageText maps a raw stage string published by parlement.ma to the
// canonical Morocco stage code. Returns "" when no mapping is known. The
...
// ParseBillsListing parses the HTML of a parlement.ma Bills listing page into
// a slice of BillCandidate records. The parser tolerates missing fields and
// never panics on malformed HTML.
...
// ParseBillDetail extracts Bill metadata from a parlement.ma Bill detail page
// (or, in the absence of a per-Bill detail page, from the listing's bill-card
// markup). Returns a single ExtractedRecord of kind "bill". If no recognisable
// Bill title is found, an error is returned.
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	...
	if len(bills) == 0 {
		...
		if title == "" {
			return nil, fmt.Errorf("morocco.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "morocco.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		...
		Sponsor:       b.Sponsor,
		PublishedAt:   parseMoroccoDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "morocco.parliament.HTMLParser",
...
// parseMoroccoDate tries the common date layouts used by parlement.ma.
// Returns the zero time if nothing parseable was provided.
func parseMoroccoDate(s string) time.Time {
	s = strings.TrimSpace(s)
	if s == "" {
		return time.Time{}
	}
	// French month names (Morocco publishes in French).
	layouts := []string{
		"2006-01-02",
		"02/01/2006",
		"02 January 2006",
	...
	for fr, en := range monthFix {
		if strings.Contains(lc, fr) {
			// Case-insensitive replace on the original string.
