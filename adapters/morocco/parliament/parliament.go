// Package parliament adapts the Parliament of Morocco source (parlement.ma).
// It implements discovery + fetch + parse for Bills and committee documents.
//
// Live site reconnaissance (2026-09-09):
//   - https://www.parlement.ma/                → HTTP 200
//   - https://www.parlement.ma/fr/projets-lois → HTTP 200 (Bills listing)
//
// The Parliament of Morocco is BICAMERAL:
//   - House of Representatives (Chambre des Représentants) — 395 members
//   - House of Councillors (Chambre des Conseillers)       — 120 members
//
// Bills originate in the House of Representatives by default; the House of
// Councillors reviews them before the Final Vote. The King promulgates Acts
...
// House code constants used to identify Morocco's bicameral chambers.
const (
	HouseCodeRepresentatives = "REPS" // lower house — Chambre des Représentants
	HouseCodeCouncillors     = "CONS" // upper house — Chambre des Conseillers
)

// Adapter adapts parlement.ma to the platform's
...
type Adapter struct {
	client    HTTPClient
	userAgent string
	// baseURL is the canonical Parliament of Morocco root.
	baseURL string
...
type BillCandidate struct {
	...
	Title string
	// House is "House of Representatives" or "House of Councillors".
	House string
	// BillNumber is the published Bill number, e.g. "Projet de Loi N° 12.24".
	BillNumber string
...
// NewAdapter constructs a parlement.ma adapter. If client is nil a default
...
func NewAdapter(client HTTPClient, userAgent string) *Adapter {
	...
	return &Adapter{
		client:    client,
		userAgent: userAgent,
		baseURL:   "https://www.parlement.ma",
		billsURL:  "https://www.parlement.ma/fr/projets-lois",
	}
}

// CountryCode returns "MA".
func (a *Adapter) CountryCode() string { return "MA" }

// Supports reports whether this adapter can handle the given URL. The
// Parliament adapter handles URLs from parlement.ma.
...
func (a *Adapter) GetOfficialSources() []SourceDefinition {
	return []SourceDefinition{
		{
			ID:             "ma-parliament-bills",
			Country:        "MA",
			Institution:    "Parliament of Morocco — Bills",
			Authority:      "primary",
			URL:            a.billsURL,
			Adapter:        "morocco.parliament",
...
// Bills default to the House of Representatives (Bills originate there by
// default); a Bill may override this via a <span class="bill-house"> in its
// bill-card markup (used for House of Councillors-originated Bills, which are rare).
func (a *Adapter) DiscoverBills(ctx context.Context) ([]BillCandidate, error) {
	body, err := a.fetchURL(ctx, a.billsURL)
	if err != nil {
		return nil, fmt.Errorf("morocco.parliament.DiscoverBills: %w", err)
	}
	cands := ParseBillsListing(string(body))
	now := time.Now().UTC()
	for i := range cands {
		if cands[i].House == "" {
			cands[i].House = "House of Representatives"
		}
		if cands[i].SourceID == "" {
...
func (a *Adapter) billCandidateToSourceItem(b BillCandidate) contracts.SourceItem {
	...
	meta := map[string]string{
		"house":       house,
		"institution": "Parliament of Morocco",
	}
	if b.BillNumber != "" {
...
func (a *Adapter) Fetch(ctx context.Context, item contracts.SourceItem) (*contracts.RawDocument, error) {
	if item.URL == "" {
		return nil, fmt.Errorf("morocco.parliament.Fetch: empty URL")
	}
...
func (a *Adapter) Parse(ctx context.Context, doc contracts.RawDocument) ([]contracts.ExtractedRecord, error) {
	...
	if doc.MimeType == "application/pdf" || strings.HasSuffix(strings.ToLower(doc.URL), ".pdf") {
		return []contracts.ExtractedRecord{{
			...
			ExtractorName: "morocco.parliament.PDFPlaceholder",
			Confidence:    0.1,
		}}, nil
	}
...
// structure data; callers should use the top-level MoroccoAdapter.
func (a *Adapter) GetLegislativeStructure(ctx context.Context) (*contracts.LegislativeStructure, error) {
	_ = ctx
	return nil, fmt.Errorf("morocco.parliament: GetLegislativeStructure not implemented on the source adapter; use the top-level MoroccoAdapter")
}
...
func (a *Adapter) GetStages(ctx context.Context) ([]contracts.StageDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("morocco.parliament: GetStages not implemented on the source adapter; use the top-level MoroccoAdapter")
}
...
func (a *Adapter) GetTerminology(ctx context.Context) ([]contracts.TermDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("morocco.parliament: GetTerminology not implemented on the source adapter; use the top-level MoroccoAdapter")
}

// NormalizeSourceItem returns a minimal SourceItem so the contract surface
// compiles. The top-level MoroccoAdapter owns its own normalizer.
...
func (a *Adapter) fetchURL(ctx context.Context, rawURL string) ([]byte, error) {
	if rawURL == "" {
		return nil, fmt.Errorf("morocco.parliament.fetchURL: empty URL")
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("morocco.parliament.fetchURL: HTTP %d for %s", resp.StatusCode, rawURL)
	}
	return io.ReadAll(resp.Body)
}

// makeSourceID derives a stable, deterministic SourceID for a Morocco
// Parliament Bill from its house and source URL.
//
//	house="House of Councillors", url="…/projets-lois/loi-donnees-personnelles.pdf"
//	→ "ma-parliament-cons-bill-loi-donnees-personnelles"
func makeSourceID(house, rawURL string) string {
	prefix := "ma-parliament-reps-bill"
	if strings.EqualFold(house, "House of Councillors") ||
		strings.Contains(strings.ToLower(house), "councillors") ||
		strings.Contains(strings.ToLower(house), "conseillers") {
		prefix = "ma-parliament-cons-bill"
	}
	slug := rawURL
...
// parlement.ma site. Production callers MUST NOT use this method.
