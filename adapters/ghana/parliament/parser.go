// Package parliament — HTML parser for the Parliament of Ghana Bills
// listing page (parliament.ghana.gov.gh/business/bills).
//
// The Parliament of Ghana publishes Bills using a card-per-bill layout that
// looks (schematically) like:
//
//      <div class="bill-card">
//        <h3 class="bill-title"><a href=".../bill.pdf">The Right to Information Bill, 2024</a></h3>
//        <div class="bill-meta">
//          <span class="bill-number">Bill No. 12 of 2024</span>
//          <span class="bill-sponsor">Minister for Justice</span>
//          <span class="bill-stage">Consideration Stage</span>
...
// stageMapping maps the lowercased substrings the Parliament of Ghana uses on
// its Bills pages to the canonical stage codes from internal/ghana_data.go.
...
var stageMapping = []struct {
	...
	{"consideration", "CONSIDERATION_STAGE"},
...
// MapStageText maps a raw stage string published by parliament.ghana.gov.gh to
// the canonical Ghana stage code. Returns "" when no mapping is known. The
...
// ParseBillsListing parses the HTML of a parliament.ghana.gov.gh Bills
...
// ParseBillDetail extracts Bill metadata from a parliament.ghana.gov.gh Bill
...
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	...
	if len(bills) == 0 {
		...
		if title == "" {
			return nil, fmt.Errorf("ghana.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "ghana.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		Kind:          "bill",
		Title:         b.Title,
		House:         "Parliament of Ghana",
		Stage:         MapStageText(b.Stage),
		Sponsor:       b.Sponsor,
		PublishedAt:   parseGhanaDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "ghana.parliament.HTMLParser",
...
// parseGhanaDate tries the common date layouts used by
// parliament.ghana.gov.gh. Returns the zero time if nothing parseable was
// provided.
func parseGhanaDate(s string) time.Time {
