// Package parliament adapts the Parliament of Ghana source
// (parliament.ghana.gov.gh). It implements discovery + fetch + parse for
// Bills, Hansard, Order Papers, Votes & Proceedings, and committee documents.
//
// Live site reconnaissance (2026-09-09):
//   - https://parliament.ghana.gov.gh/               → HTTP 200
//   - https://parliament.ghana.gov.gh/business/bills  → HTTP 200 (Bills listing)
//
...
// Adapter adapts parliament.ghana.gov.gh to the platform's
...
type Adapter struct {
	client    HTTPClient
	userAgent string
	// baseURL is the canonical Parliament of Ghana root.
	baseURL string
...
type BillCandidate struct {
	...
	Title string
	// House is always "Parliament of Ghana" (Ghana is unicameral).
	House string
...
// NewAdapter constructs a parliament.ghana.gov.gh adapter. If client is nil a
...
func NewAdapter(client HTTPClient, userAgent string) *Adapter {
	...
	return &Adapter{
		client:    client,
		userAgent: userAgent,
		baseURL:   "https://parliament.ghana.gov.gh",
		billsURL:  "https://parliament.ghana.gov.gh/business/bills",
	}
}

// CountryCode returns "GH".
func (a *Adapter) CountryCode() string { return "GH" }

// Supports reports whether this adapter can handle the given URL. The
// Parliament adapter handles URLs from parliament.ghana.gov.gh.
func (a *Adapter) Supports(u string) bool {
	for _, host := range []string{
		"parliament.ghana.gov.gh",
		"parliament.gh",
	} {
...
func (a *Adapter) GetOfficialSources() []SourceDefinition {
	return []SourceDefinition{
		{
			ID:             "gh-parliament-bills",
			Country:        "GH",
			Institution:    "Parliament of Ghana — Bills",
			Authority:      "primary",
			URL:            a.billsURL,
			Adapter:        "ghana.parliament",
...
func (a *Adapter) DiscoverBills(ctx context.Context) ([]BillCandidate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("ghana.parliament.DiscoverBills: %w", err)
	}
	cands := ParseBillsListing(string(body))
	now := time.Now().UTC()
	for i := range cands {
		cands[i].House = "Parliament of Ghana"
		if cands[i].SourceID == "" {
...
func (a *Adapter) Fetch(ctx context.Context, item contracts.SourceItem) (*contracts.RawDocument, error) {
	if item.URL == "" {
		return nil, fmt.Errorf("ghana.parliament.Fetch: empty URL")
	}
...
func (a *Adapter) Parse(ctx context.Context, doc contracts.RawDocument) ([]contracts.ExtractedRecord, error) {
	...
	if doc.MimeType == "application/pdf" || strings.HasSuffix(strings.ToLower(doc.URL), ".pdf") {
		return []contracts.ExtractedRecord{{
			...
			ExtractorName: "ghana.parliament.PDFPlaceholder",
			Confidence:    0.1,
		}}, nil
	}
...
// structure data; callers should use the top-level GhanaAdapter.
func (a *Adapter) GetLegislativeStructure(ctx context.Context) (*contracts.LegislativeStructure, error) {
	_ = ctx
	return nil, fmt.Errorf("ghana.parliament: GetLegislativeStructure not implemented on the source adapter; use the top-level GhanaAdapter")
}
...
func (a *Adapter) GetStages(ctx context.Context) ([]contracts.StageDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("ghana.parliament: GetStages not implemented on the source adapter; use the top-level GhanaAdapter")
}
...
func (a *Adapter) GetTerminology(ctx context.Context) ([]contracts.TermDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("ghana.parliament: GetTerminology not implemented on the source adapter; use the top-level GhanaAdapter")
}

// NormalizeSourceItem returns a minimal SourceItem so the contract surface
// compiles. The top-level GhanaAdapter owns its own normalizer.
...
func (a *Adapter) fetchURL(ctx context.Context, rawURL string) ([]byte, error) {
	if rawURL == "" {
		return nil, fmt.Errorf("ghana.parliament.fetchURL: empty URL")
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("ghana.parliament.fetchURL: HTTP %d for %s", resp.StatusCode, rawURL)
	}
	return readAll(resp.Body)
}

// makeSourceID derives a stable, deterministic SourceID for a Ghana
// Parliament Bill from its source URL.
func makeSourceID(rawURL string) string {
	prefix := "gh-parliament-bill"
	slug := rawURL
...
// readAll reads all bytes from r in 4 KB chunks.
...
// parliament.ghana.gov.gh site. Production callers MUST NOT use this method.
