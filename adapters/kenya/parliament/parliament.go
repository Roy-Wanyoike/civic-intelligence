...
//
// Live site reconnaissance (updated 2026-09-21):
...
//   - https://www.parliament.go.ke/the-national-assembly/house-business/hansard           → HTTP 200
//   - https://www.parliament.go.ke/the-national-assembly/house-business/order-paper        → HTTP 200
//   - https://www.parliament.go.ke/the-national-assembly/house-business/votes-proceeding    → HTTP 200 (singular)
//   - https://www.parliament.go.ke/the-senate/Hansard                                       → HTTP 200 (capital H)
//   - https://www.parliament.go.ke/the-senate/house-business/order-paper                    → HTTP 200 (inferred)
//   - https://www.parliament.go.ke/the-senate/house-business/votes-proceeding              → HTTP 200 (inferred)
//   - https://www.parliament.go.ke/the-national-assembly/committees                          → HTTP 200 (committee index)
//   - https://www.parliament.go.ke/the-senate/committees/senate-committees                   → HTTP 200 (senate committee index)
//   - https://www.parliament.go.ke/bills                                                     → HTTP 404 (NOT a real path)
//   - https://www.parliament.go.ke/hansard                                                    → HTTP 404 (NOT a real path — old TODO)
//   - https://www.parliament.go.ke/order-papers                                              → HTTP 404 (NOT a real path — old TODO)
//   - https://www.parliament.go.ke/votes-and-proceedings                                      → HTTP 404 (NOT a real path — old TODO)
//   - https://www.parliament.go.ke/committees                                                 → HTTP 404 (use /the-national-assembly/committees)
//
// The Bills, Hansard, Order Paper, and Votes & Proceedings listing pages all
// use the same Drupal Views structure: a paginated table (25 rows per page,
// ?page=N) with each row pointing to a PDF under /sites/default/files/YYYY-MM/.
// The parser (ParsePDFListing) handles all four document types uniformly.
//
// Committee reports are organised per-committee rather than as a single
// archive — the DiscoverCommittees crawler fetches the committee index,
// then for each committee fetches the per-committee page and extracts the
// report links from div.field--name-field-committee-report.
//
// The Bills listing pages (Bills only, not the other doc types) use the
// "post-block" structure (see ParseBillsListing):
//
...
type Adapter struct {
        ...
        // naHansardURL is the National Assembly Hansard listing page.
        naHansardURL string
        // senateHansardURL is the Senate Hansard listing page (capital H).
        senateHansardURL string
        // naOrderPaperURL is the National Assembly Order Paper listing page.
        naOrderPaperURL string
        // senateOrderPaperURL is the Senate Order Paper listing page.
        senateOrderPaperURL string
        // naVotesURL is the National Assembly Votes & Proceedings listing page
        // (note: parliament.go.ke uses the singular form "votes-proceeding").
        naVotesURL string
        // senateVotesURL is the Senate Votes & Proceedings listing page.
        senateVotesURL string
        // naCommitteesURL is the National Assembly committee index page.
        naCommitteesURL string
        // senateCommitteesURL is the Senate committee index page.
        senateCommitteesURL string
}
...
func NewAdapter(client HTTPClient, userAgent string) *Adapter {
        ...
        return &Adapter{
                ...
                naHansardURL:       "https://www.parliament.go.ke/the-national-assembly/house-business/hansard",
                senateHansardURL:   "https://www.parliament.go.ke/the-senate/Hansard",
                naOrderPaperURL:    "https://www.parliament.go.ke/the-national-assembly/house-business/order-paper",
                senateOrderPaperURL: "https://www.parliament.go.ke/the-senate/house-business/order-paper",
                naVotesURL:         "https://www.parliament.go.ke/the-national-assembly/house-business/votes-proceeding",
                senateVotesURL:     "https://www.parliament.go.ke/the-senate/house-business/votes-proceeding",
                naCommitteesURL:    "https://www.parliament.go.ke/the-national-assembly/committees",
                senateCommitteesURL: "https://www.parliament.go.ke/the-senate/committees/senate-committees",
        }
}
...
func (a *Adapter) GetOfficialSources() []SourceDefinition {
        return []SourceDefinition{
                ...
                {
                        ID:             "ke-parliament-na-hansard",
                        Country:        "KE",
                        Institution:    "Parliament of Kenya — National Assembly Hansard",
                        Authority:      "primary",
                        URL:            a.naHansardURL,
                        Adapter:        "kenya.parliament",
                ...
                {
                        ID:             "ke-parliament-senate-hansard",
                        Country:        "KE",
                        Institution:    "Parliament of Kenya — Senate Hansard",
                        Authority:      "primary",
                        URL:            a.senateHansardURL,
                        Adapter:        "kenya.parliament",
                        DocumentTypes:  []string{"hansard"},
                        CrawlFrequency: 24 * time.Hour,
                },
                {
                        ID:             "ke-parliament-na-order-paper",
                        Country:        "KE",
                        Institution:    "Parliament of Kenya — National Assembly Order Paper",
                        Authority:      "primary",
                        URL:            a.naOrderPaperURL,
                        Adapter:        "kenya.parliament",
                        DocumentTypes:  []string{"order_paper"},
                        CrawlFrequency: 24 * time.Hour,
                },
                {
                        ID:             "ke-parliament-senate-order-paper",
                        Country:        "KE",
                        Institution:    "Parliament of Kenya — Senate Order Paper",
                        Authority:      "primary",
                        URL:            a.senateOrderPaperURL,
                        Adapter:        "kenya.parliament",
                        DocumentTypes:  []string{"order_paper"},
                        CrawlFrequency: 24 * time.Hour,
                },
                {
                        ID:             "ke-parliament-na-votes",
                        Country:        "KE",
                        Institution:    "Parliament of Kenya — National Assembly Votes & Proceedings",
                        Authority:      "primary",
                        URL:            a.naVotesURL,
                        Adapter:        "kenya.parliament",
                        DocumentTypes:  []string{"votes_proceedings"},
                        CrawlFrequency: 24 * time.Hour,
                },
                {
                        ID:             "ke-parliament-senate-votes",
                        Country:        "KE",
                        Institution:    "Parliament of Kenya — Senate Votes & Proceedings",
                        Authority:      "primary",
                        URL:            a.senateVotesURL,
                        Adapter:        "kenya.parliament",
                        DocumentTypes:  []string{"votes_proceedings"},
                        CrawlFrequency: 24 * time.Hour,
                },
                {
                        ID:             "ke-parliament-na-committees",
                        Country:        "KE",
                        Institution:    "Parliament of Kenya — National Assembly Committees",
                        Authority:      "primary",
                        URL:            a.naCommitteesURL,
                        Adapter:        "kenya.parliament",
                        DocumentTypes:  []string{"committee_report"},
                        CrawlFrequency: 24 * time.Hour,
                },
                {
                        ID:             "ke-parliament-senate-committees",
                        Country:        "KE",
                        Institution:    "Parliament of Kenya — Senate Committees",
                        Authority:      "primary",
                        URL:            a.senateCommitteesURL,
                        Adapter:        "kenya.parliament",
                        DocumentTypes:  []string{"committee_report"},
                        CrawlFrequency: 24 * time.Hour,
                },
