// Package parliament — HTML parser for the Parliament of Malawi Bills
// listing page (parliament.gov.mw/bills).
//
// The Parliament of Malawi publishes Bills using a card-per-bill layout that
...
// stageMapping maps the lowercased substrings the Parliament of Malawi uses on
// its Bills pages to the canonical stage codes from internal/malawi_data.go.
...
// MapStageText maps a raw stage string published by parliament.gov.mw to the
// canonical Malawi stage code. Returns "" when no mapping is known. The
...
// ParseBillsListing parses the HTML of a parliament.gov.mw Bills listing
...
// ParseBillDetail extracts Bill metadata from a parliament.gov.mw Bill detail
...
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	...
	if len(bills) == 0 {
		...
		if title == "" {
			return nil, fmt.Errorf("malawi.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "malawi.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		Kind:          "bill",
		Title:         b.Title,
		House:         "National Assembly of Malawi",
		Stage:         MapStageText(b.Stage),
		Sponsor:       b.Sponsor,
		PublishedAt:   parseMalawiDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "malawi.parliament.HTMLParser",
...
// parseMalawiDate tries the common date layouts used by parliament.gov.mw.
// Returns the zero time if nothing parseable was provided.
func parseMalawiDate(s string) time.Time {
