// Package parliament adapts the Parliament of Malawi source
// (parliament.gov.mw). It implements discovery + fetch + parse for Bills,
// Hansard, Order Papers, Votes & Proceedings, and committee documents.
//
// Live site reconnaissance (2026-09-09):
//   - https://www.parliament.gov.mw/            → HTTP 200
//   - https://www.parliament.gov.mw/bills       → HTTP 200 (Bills listing)
//
// The Parliament of Malawi is UNICAMERAL — there is only one House, the
...
// Adapter adapts parliament.gov.mw to the platform's
...
type Adapter struct {
	client    HTTPClient
	userAgent string
	// baseURL is the canonical Parliament of Malawi root.
	baseURL string
...
type BillCandidate struct {
	...
	Title string
	// House is always "National Assembly of Malawi" (Malawi is unicameral).
	House string
...
// NewAdapter constructs a parliament.gov.mw adapter. If client is nil a
...
func NewAdapter(client HTTPClient, userAgent string) *Adapter {
	...
	return &Adapter{
		client:    client,
		userAgent: userAgent,
		baseURL:   "https://www.parliament.gov.mw",
		billsURL:  "https://www.parliament.gov.mw/bills",
	}
}

// CountryCode returns "MW".
func (a *Adapter) CountryCode() string { return "MW" }

// Supports reports whether this adapter can handle the given URL. The
// Parliament adapter handles URLs from parliament.gov.mw.
	...
	return []SourceDefinition{
		{
			...
			Country:        "MW",
			Institution:    "Parliament of Malawi — Bills",
			Authority:      "primary",
			URL:            a.billsURL,
			Adapter:        "malawi.parliament",
...
func (a *Adapter) DiscoverBills(ctx context.Context) ([]BillCandidate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("malawi.parliament.DiscoverBills: %w", err)
	}
	cands := ParseBillsListing(string(body))
	now := time.Now().UTC()
	for i := range cands {
		cands[i].House = "National Assembly of Malawi"
		if cands[i].SourceID == "" {
...
func (a *Adapter) billCandidateToSourceItem(b BillCandidate) contracts.SourceItem {
	meta := map[string]string{
		"house":       b.House,
		"institution": "Parliament of Malawi",
	}
	if b.BillNumber != "" {
...
func (a *Adapter) Fetch(ctx context.Context, item contracts.SourceItem) (*contracts.RawDocument, error) {
	if item.URL == "" {
		return nil, fmt.Errorf("malawi.parliament.Fetch: empty URL")
	}
...
func (a *Adapter) Parse(ctx context.Context, doc contracts.RawDocument) ([]contracts.ExtractedRecord, error) {
	...
	if doc.MimeType == "application/pdf" || strings.HasSuffix(strings.ToLower(doc.URL), ".pdf") {
		return []contracts.ExtractedRecord{{
			...
			ExtractorName: "malawi.parliament.PDFPlaceholder",
			Confidence:    0.1,
		}}, nil
	}
...
// structure data; callers should use the top-level MalawiAdapter.
func (a *Adapter) GetLegislativeStructure(ctx context.Context) (*contracts.LegislativeStructure, error) {
	_ = ctx
	return nil, fmt.Errorf("malawi.parliament: GetLegislativeStructure not implemented on the source adapter; use the top-level MalawiAdapter")
}
...
func (a *Adapter) GetStages(ctx context.Context) ([]contracts.StageDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("malawi.parliament: GetStages not implemented on the source adapter; use the top-level MalawiAdapter")
}
...
func (a *Adapter) GetTerminology(ctx context.Context) ([]contracts.TermDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("malawi.parliament: GetTerminology not implemented on the source adapter; use the top-level MalawiAdapter")
}

// NormalizeSourceItem returns a minimal SourceItem so the contract surface
// compiles. The top-level MalawiAdapter owns its own normalizer.
...
func (a *Adapter) fetchURL(ctx context.Context, rawURL string) ([]byte, error) {
	if rawURL == "" {
		return nil, fmt.Errorf("malawi.parliament.fetchURL: empty URL")
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("malawi.parliament.fetchURL: HTTP %d for %s", resp.StatusCode, rawURL)
	}
	return io.ReadAll(resp.Body)
}

// makeSourceID derives a stable, deterministic SourceID for a Malawi
...
// parliament.gov.mw site. Production callers MUST NOT use this method.
