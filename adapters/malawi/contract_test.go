// Package malawi_test verifies that the Malawi adapter satisfies the global
// contracts.LegislativeSourceAdapter interface and that all Malawi-specific
...
var _ contracts.LegislativeSourceAdapter = (*malawi.MalawiAdapter)(nil)

func TestMalawiAdapter_SatisfiesInterface(t *testing.T) {
	a := malawi.NewMalawiAdapter()
	assert.NotNil(t, a)
}

func TestMalawiAdapter_CountryCode(t *testing.T) {
	a := malawi.NewMalawiAdapter()
	assert.Equal(t, "MW", a.CountryCode())
}

func TestMalawiAdapter_Supports(t *testing.T) {
	a := malawi.NewMalawiAdapter()
	assert.True(t, a.Supports("https://www.parliament.gov.mw/bills"))
	assert.True(t, a.Supports("https://parliament.gov.mw/bills/2024/edu"))
	assert.False(t, a.Supports("https://parliament.go.ke"))
	assert.False(t, a.Supports("https://www.parliament.go.ug"))
}

func TestMalawiBillStages_AllHaveCountry(t *testing.T) {
	stages := internal.MalawiBillStages
	assert.NotEmpty(t, stages)
	for _, s := range stages {
		assert.Equal(t, contracts.Country("MW"), s.Country, "stage %q has wrong country", s.Code)
...
func TestMalawiBillStages_TerminalStagesExist(t *testing.T) {
	stages := internal.MalawiBillStages
...
func TestMalawiBillStages_StageChainIsValid(t *testing.T) {
	stages := internal.MalawiBillStages
	byCode := map[string]contracts.StageDefinition{}
	for _, s := range stages {
		byCode[s.Code] = s
	}
	// Verify Malawi's canonical Bill flow:
...
func TestMalawiBillStages_AllowedNextReferencesValid(t *testing.T) {
	stages := internal.MalawiBillStages
	byCode := map[string]contracts.StageDefinition{}
	for _, s := range stages {
		byCode[s.Code] = s
...
func TestMalawiLegislativeStructure_Unicameral(t *testing.T) {
	s := internal.MalawiLegislativeStructure()
	assert.Equal(t, contracts.Country("MW"), s.Country)
	assert.Len(t, s.Houses, 1, "Malawi should be unicameral (one House)")
	assert.Equal(t, "National Assembly of Malawi", s.Houses[0].Name)
	assert.Equal(t, contracts.HouseTypeSingle, s.Houses[0].Type)
}

func TestMalawiLegislativeStructure_MemberCount(t *testing.T) {
	s := internal.MalawiLegislativeStructure()
	assert.Len(t, s.Houses, 1)
	assert.Equal(t, 193, s.Houses[0].Members, "National Assembly of Malawi has 193 elected MPs")
}

func TestMalawiLegislativeStructure_TermDays(t *testing.T) {
	s := internal.MalawiLegislativeStructure()
	assert.Len(t, s.Houses, 1)
	// Malawi's parliamentary term is five years per Article 50 of the 1994 Constitution.
	assert.Equal(t, 5*365, s.Houses[0].TermDays)
}

func TestMalawiTerminology_AllHaveCountry(t *testing.T) {
	terms := internal.MalawiTerminology
	assert.GreaterOrEqual(t, len(terms), 20, "should have at least 20 terms")
	for _, term := range terms {
		assert.Equal(t, contracts.Country("MW"), term.Country, "term %q has wrong country", term.Term)
		assert.NotEmpty(t, term.Term)
		assert.NotEmpty(t, term.SimpleExplanation)
	}
}

func TestMalawiTerminology_ContainsCommitteeStage(t *testing.T) {
	terms := internal.MalawiTerminology
	found := false
	for _, term := range terms {
	...
	assert.True(t, found, "Malawi terminology must include the 'Committee Stage' term")
}

func TestMalawiAdapter_GetStages(t *testing.T) {
	a := malawi.NewMalawiAdapter()
...
func TestMalawiAdapter_GetTerminology(t *testing.T) {
	a := malawi.NewMalawiAdapter()
...
func TestMalawiAdapter_GetLegislativeStructure(t *testing.T) {
	a := malawi.NewMalawiAdapter()
	s, err := a.GetLegislativeStructure(context.Background())
	assert.NoError(t, err)
	assert.NotNil(t, s)
	assert.Equal(t, contracts.Country("MW"), s.Country)
	assert.Len(t, s.Houses, 1, "Malawi is unicameral")
}

func TestMalawiAdapter_NormalizeSourceItem(t *testing.T) {
	a := malawi.NewMalawiAdapter()
	item, err := a.NormalizeSourceItem(map[string]any{
		"url":   "https://www.parliament.gov.mw/bills/2024/edu",
		"title": "The Education (Amendment) Bill, 2024",
	})
	assert.NoError(t, err)
	assert.Equal(t, "https://www.parliament.gov.mw/bills/2024/edu", item.URL)
	...
	assert.Equal(t, "MW", item.CountryCode)
}

func TestMalawiAdapter_DiscoverReturnsEmptyWithoutError(t *testing.T) {
	a := malawi.NewMalawiAdapter()
	// Without overriding the bills URL, Discover will attempt to hit the
	// real parliament.gov.mw site, which is unreachable from the sandbox.
...
func TestAdapter_DiscoverBills_ViaMockServer(t *testing.T) {
	html := loadMalawiFixture(t, "bills.html")
	...
	for i, item := range items {
		assert.Equal(t, "MW", item.CountryCode, "item %d has wrong CountryCode", i)
		...
		assert.Equal(t, "National Assembly of Malawi", item.House, "item %d has wrong House", i)
		assert.Equal(t, "National Assembly of Malawi", item.Metadata["house"], "item %d has wrong house metadata", i)
		assert.Equal(t, "Parliament of Malawi", item.Metadata["institution"], "item %d has wrong institution metadata", i)
	}
...
// TestMalawiSampleBills_CountAndShape verifies the Malawi sample Bills
// registry has 5 entries that all reference parliament.gov.mw.
func TestMalawiSampleBills_CountAndShape(t *testing.T) {
	bills := internal.MalawiSampleBills
	require.Len(t, bills, 5, "MalawiSampleBills should have exactly 5 entries")
	for i, b := range bills {
		...
		assert.True(t, strings.Contains(b.URL, "parliament.gov.mw"),
			"sample bill %d URL should reference parliament.gov.mw", i)
	}
}

// loadMalawiFixture reads a testdata/ fixture into a string.
func loadMalawiFixture(t *testing.T, name string) string {
	t.Helper()
	candidates := []string{
