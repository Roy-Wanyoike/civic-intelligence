// Package parliament — HTML parser for the Egyptian Parliament Bills listing
// pages (parliament.eg/house/bills and parliament.eg/senate/bills).
//
// The Egyptian Parliament publishes Bills using a card-per-bill layout that
// looks (schematically) like:
//
//	<div class="bill-card">
//	  <h3 class="bill-title"><a href=".../bill.pdf">New Investment Law</a></h3>
//	  <div class="bill-meta">
//	    <span class="bill-number">Bill No. 18/2024</span>
//	    <span class="bill-sponsor">Hon. Minister of Investment and International Cooperation</span>
//	    <span class="bill-stage">Presidential Ratification</span>
...
// stageMapping maps the lowercased substrings the Egyptian Parliament uses on
// its Bills pages to the canonical stage codes from internal/egypt_data.go.
...
var stageMapping = []struct {
	...
	{"presidential ratification", "PRESIDENTIAL_RATIFICATION"},
	{"ratification", "PRESIDENTIAL_RATIFICATION"},
	{"ratified", "PRESIDENTIAL_RATIFICATION"},
	{"assent", "PRESIDENTIAL_RATIFICATION"},

	{"publication", "PUBLICATION"},
	{"published", "PUBLICATION"},
	{"gazetted", "PUBLICATION"},
	{"in force", "PUBLICATION"},
...
// MapStageText maps the Egyptian Parliament's raw stage text (e.g.,
// "Presidential Ratification", "Senate Review", "Publication") to the
// canonical Egypt stage codes. The match is case-insensitive and substring-
// based. Returns "" if the input does not match a known stage.
...
// ParseBillsListing parses the HTML of a parliament.eg Bills listing page
...
// ParseBillDetail extracts Bill metadata from a parliament.eg Bill detail
...
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	...
	if len(bills) == 0 {
		...
		if title == "" {
			return nil, fmt.Errorf("egypt.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "egypt.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		...
		Sponsor:       b.Sponsor,
		PublishedAt:   parseEgyptDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "egypt.parliament.HTMLParser",
...
// parseEgyptDate tries the common date layouts used by parliament.eg.
// Returns the zero time if nothing parseable was provided.
func parseEgyptDate(s string) time.Time {
