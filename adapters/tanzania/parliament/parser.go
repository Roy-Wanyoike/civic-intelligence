// Package parliament — HTML parser for the Parliament of Tanzania Bills
// listing page (parliament.go.tz/bunge/).
//
// The Parliament of Tanzania publishes Bills on the Bunge website using a
// card-per-bill layout that looks (schematically) like:
//
//      <div class="bill-card">
//        <h3 class="bill-title"><a href=".../bill.pdf">The Written Laws Bill, 2023</a></h3>
//        <div class="bill-meta">
//          <span class="bill-number">Bill No. 7 of 2023</span>
//          <span class="bill-sponsor">Minister for Justice</span>
//          <span class="bill-stage">Second Reading</span>
//          <span class="bill-date">15 March 2023</span>
//        </div>
//      </div>
//
...
// stageMapping maps the lowercased substrings the Bunge uses on its Bills
// pages to the canonical stage codes from internal/tanzania_data.go. Order
...
// MapStageText maps a raw stage string published by parliament.go.tz to the
// canonical Tanzania stage code. Returns "" when no mapping is known. The
...
// ParseBillsListing parses the HTML of a parliament.go.tz Bills listing page
// into a slice of BillCandidate records. The parser tolerates missing fields
// and never panics on malformed HTML.
...
// ParseBillDetail extracts Bill metadata from a parliament.go.tz Bill detail
// page (or, in the absence of a per-Bill detail page, from the listing's
// bill-card markup). Returns a single ExtractedRecord of kind "bill".
//
// If the HTML carries no recognisable Bill title, an error is returned so the
// caller can quarantine the document rather than silently dropping it.
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	bills := ParseBillsListing(pageHTML)
	if len(bills) == 0 {
		// Fall back to the page's first <a> as the title — this handles
		// cases where the detail page uses a different layout than the
		// listing page but still links to the Bill PDF.
		title, pdfURL := firstAnchor(pageHTML)
		if title == "" {
			return nil, fmt.Errorf("tanzania.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "tanzania.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		Kind:          "bill",
		Title:         b.Title,
		House:         "Bunge la Tanzania",
		Stage:         MapStageText(b.Stage),
		Sponsor:       b.Sponsor,
		PublishedAt:   parseTanzaniaDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "tanzania.parliament.HTMLParser",
...
// parseTanzaniaDate tries the common date layouts used by parliament.go.tz.
// Returns the zero time if nothing parseable was provided.
func parseTanzaniaDate(s string) time.Time {
