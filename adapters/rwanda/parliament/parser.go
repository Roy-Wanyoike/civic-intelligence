// Package parliament — HTML parser for the Parliament of Rwanda Bills
// listing pages (parliament.gov.rw/chamber-of-deputies/bills and
// parliament.gov.rw/senate/bills).
//
// The Parliament of Rwanda publishes Bills using a card-per-bill layout that
// looks (schematically) like:
//
//	<div class="bill-card">
//	  <h3 class="bill-title"><a href=".../bill.pdf">Law on the Protection of Personal Data and Privacy</a></h3>
//	  <div class="bill-meta">
//	    <span class="bill-number">Law No. 058/2021</span>
//	    <span class="bill-sponsor">Hon. Minister of ICT and Innovation</span>
//	    <span class="bill-stage">Presidential Assent</span>
//	    <span class="bill-date">15 October 2021</span>
//	  </div>
//	</div>
//
...
// stageMapping maps the lowercased substrings the Parliament of Rwanda uses
// on its Bills pages to the canonical stage codes from
// internal/rwanda_data.go. Order matters: more specific patterns MUST come
// first.
...
// MapStageText maps Parliament of Rwanda's raw stage text (e.g., "Second
// Reading", "Senate Review", "Presidential Assent") to the canonical Rwanda
// stage codes (FIRST_READING, SECOND_READING, …, PRESIDENTIAL_ASSENT, …).
// The match is case-insensitive and substring-based. Returns "" if the input
// does not match a known stage.
...
// ParseBillsListing parses the HTML of a parliament.gov.rw Bills listing page
// (Chamber of Deputies or Senate) into a slice of BillCandidate records. The
// parser tolerates missing fields and never panics on malformed HTML.
...
// ParseBillDetail extracts Bill metadata from a parliament.gov.rw Bill detail
...
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	...
	if len(bills) == 0 {
		...
		if title == "" {
			return nil, fmt.Errorf("rwanda.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "rwanda.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		...
		Sponsor:       b.Sponsor,
		PublishedAt:   parseRwandaDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "rwanda.parliament.HTMLParser",
...
// parseRwandaDate tries the common date layouts used by parliament.gov.rw.
// Returns the zero time if nothing parseable was provided.
func parseRwandaDate(s string) time.Time {
