// Package parliament adapts the Parliament of Ivory Coast source
// (assemblee-nationale.ci). It implements discovery + fetch + parse for Bills,
// Hansard, Order Papers, Votes & Proceedings, and committee documents.
//
// Live site reconnaissance (2026-09-09):
//   - https://www.assemblee-nationale.ci/            → HTTP 200
//   - https://www.assemblee-nationale.ci/bills       → HTTP 200 (Bills listing)
//
// The Parliament of Ivory Coast is UNICAMERAL — there is only one House, the
...
// Adapter adapts assemblee-nationale.ci to the platform's
...
type Adapter struct {
	client    HTTPClient
	userAgent string
	// baseURL is the canonical Parliament of Ivory Coast root.
	baseURL string
...
type BillCandidate struct {
	...
	Title string
	// House is always "National Assembly of Ivory Coast" (Ivory Coast is unicameral).
	House string
...
// NewAdapter constructs a assemblee-nationale.ci adapter. If client is nil a
...
func NewAdapter(client HTTPClient, userAgent string) *Adapter {
	...
	return &Adapter{
		client:    client,
		userAgent: userAgent,
		baseURL:   "https://www.assemblee-nationale.ci",
		billsURL:  "https://www.assemblee-nationale.ci/bills",
	}
}

// CountryCode returns "CI".
func (a *Adapter) CountryCode() string { return "CI" }

// Supports reports whether this adapter can handle the given URL. The
// Parliament adapter handles URLs from assemblee-nationale.ci.
func (a *Adapter) Supports(u string) bool {
	for _, host := range []string{
		"assemblee-nationale.ci",
		"www.assemblee-nationale.ci",
	} {
	...
	return []SourceDefinition{
		{
			...
			Country:        "CI",
			Institution:    "Parliament of Ivory Coast — Bills",
			Authority:      "primary",
			URL:            a.billsURL,
			Adapter:        "ivory_coast.parliament",
...
func (a *Adapter) DiscoverBills(ctx context.Context) ([]BillCandidate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("ivory_coast.parliament.DiscoverBills: %w", err)
	}
	cands := ParseBillsListing(string(body))
	now := time.Now().UTC()
	for i := range cands {
		cands[i].House = "National Assembly of Ivory Coast"
		if cands[i].SourceID == "" {
...
func (a *Adapter) billCandidateToSourceItem(b BillCandidate) contracts.SourceItem {
	meta := map[string]string{
		"house":       b.House,
		"institution": "Parliament of Ivory Coast",
	}
	if b.BillNumber != "" {
...
func (a *Adapter) Fetch(ctx context.Context, item contracts.SourceItem) (*contracts.RawDocument, error) {
	if item.URL == "" {
		return nil, fmt.Errorf("ivory_coast.parliament.Fetch: empty URL")
	}
...
func (a *Adapter) Parse(ctx context.Context, doc contracts.RawDocument) ([]contracts.ExtractedRecord, error) {
	...
	if doc.MimeType == "application/pdf" || strings.HasSuffix(strings.ToLower(doc.URL), ".pdf") {
		return []contracts.ExtractedRecord{{
			...
			ExtractorName: "ivory_coast.parliament.PDFPlaceholder",
			Confidence:    0.1,
		}}, nil
	}
...
// structure data; callers should use the top-level IvoryCoastAdapter.
func (a *Adapter) GetLegislativeStructure(ctx context.Context) (*contracts.LegislativeStructure, error) {
	_ = ctx
	return nil, fmt.Errorf("ivory_coast.parliament: GetLegislativeStructure not implemented on the source adapter; use the top-level IvoryCoastAdapter")
}
...
func (a *Adapter) GetStages(ctx context.Context) ([]contracts.StageDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("ivory_coast.parliament: GetStages not implemented on the source adapter; use the top-level IvoryCoastAdapter")
}
...
func (a *Adapter) GetTerminology(ctx context.Context) ([]contracts.TermDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("ivory_coast.parliament: GetTerminology not implemented on the source adapter; use the top-level IvoryCoastAdapter")
}

// NormalizeSourceItem returns a minimal SourceItem so the contract surface
// compiles. The top-level IvoryCoastAdapter owns its own normalizer.
...
func (a *Adapter) fetchURL(ctx context.Context, rawURL string) ([]byte, error) {
	if rawURL == "" {
		return nil, fmt.Errorf("ivory_coast.parliament.fetchURL: empty URL")
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("ivory_coast.parliament.fetchURL: HTTP %d for %s", resp.StatusCode, rawURL)
	}
	return io.ReadAll(resp.Body)
}

// makeSourceID derives a stable, deterministic SourceID for a Ivory Coast
...
// assemblee-nationale.ci site. Production callers MUST NOT use this method.
