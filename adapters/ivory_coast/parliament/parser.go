// Package parliament — HTML parser for the Parliament of Ivory Coast Bills
// listing page (assemblee-nationale.ci/bills).
//
// The Parliament of Ivory Coast publishes Bills using a card-per-bill layout that
...
// stageMapping maps the lowercased substrings the Parliament of Ivory Coast uses on
// its Bills pages to the canonical stage codes from internal/ivory_coast_data.go.
...
// MapStageText maps a raw stage string published by assemblee-nationale.ci to the
// canonical Ivory Coast stage code. Returns "" when no mapping is known. The
...
// ParseBillsListing parses the HTML of a assemblee-nationale.ci Bills listing
...
// ParseBillDetail extracts Bill metadata from a assemblee-nationale.ci Bill detail
...
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	...
	if len(bills) == 0 {
		...
		if title == "" {
			return nil, fmt.Errorf("ivory_coast.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "ivory_coast.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		Kind:          "bill",
		Title:         b.Title,
		House:         "National Assembly of Ivory Coast",
		Stage:         MapStageText(b.Stage),
		Sponsor:       b.Sponsor,
		PublishedAt:   parseIvoryCoastDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "ivory_coast.parliament.HTMLParser",
...
// parseIvoryCoastDate tries the common date layouts used by assemblee-nationale.ci.
// Returns the zero time if nothing parseable was provided.
func parseIvoryCoastDate(s string) time.Time {
