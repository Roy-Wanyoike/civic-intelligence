...
// PDFs.
//
// This package is a SKELETON implementation: Discover / Parse return empty
// results and a low-confidence placeholder respectively. Fetch works for any
// URL the adapter Supports(). Issue #CI-AD-ZA-001 tracks the full HTML
// parser implementation against saved test fixtures.
package parliament
...
//
// SKELETON: returns an empty slice. Issue #CI-AD-ZA-001 tracks the full HTML
// parser implementation against saved test fixtures.
func (a *Adapter) DiscoverBills(ctx context.Context) ([]BillCandidate, error) {
        // TODO(issue #CI-AD-ZA-001): implement HTML parser for
        // https://www.parliament.gov.za/bills-and-laws
        return []BillCandidate{}, nil
}
...
// parsed for stage + sponsor + publication date (TODO: full parser). PDFs are
...
func (a *Adapter) Parse(ctx context.Context, doc contracts.RawDocument) ([]contracts.ExtractedRecord, error) {
        ...
        // TODO(issue #CI-AD-ZA-001): implement HTML parser for parliament.gov.za
        // Bill detail pages. Until then, return a low-confidence placeholder so the
        // caller can see the source URL was reached.
        return []contracts.ExtractedRecord{{
                Kind:          "bill",
                SourceURL:     doc.URL,
                RetrievedAt:   doc.RetrievedAt,
                ExtractorName: "south_africa.parliament.HTMLPlaceholder",
                Confidence:    0.1,
        }}, nil
}
