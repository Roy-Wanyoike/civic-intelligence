// Package parliament — HTML parser for the Parliament of Cameroon Bills
// listing page (parliament.cm/bills).
//
// The Parliament of Cameroon publishes Bills using a card-per-bill layout that
...
// stageMapping maps the lowercased substrings the Parliament of Cameroon uses on
// its Bills pages to the canonical stage codes from internal/cameroon_data.go.
...
// MapStageText maps a raw stage string published by parliament.cm to the
// canonical Cameroon stage code. Returns "" when no mapping is known. The
...
// ParseBillsListing parses the HTML of a parliament.cm Bills listing
...
// ParseBillDetail extracts Bill metadata from a parliament.cm Bill detail
...
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	...
	if len(bills) == 0 {
		...
		if title == "" {
			return nil, fmt.Errorf("cameroon.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "cameroon.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		Kind:          "bill",
		Title:         b.Title,
		House:         "National Assembly of Cameroon",
		Stage:         MapStageText(b.Stage),
		Sponsor:       b.Sponsor,
		PublishedAt:   parseCameroonDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "cameroon.parliament.HTMLParser",
...
// parseCameroonDate tries the common date layouts used by parliament.cm.
// Returns the zero time if nothing parseable was provided.
func parseCameroonDate(s string) time.Time {
