// Package parliament adapts the Parliament of Cameroon source
// (parliament.cm). It implements discovery + fetch + parse for Bills,
// Hansard, Order Papers, Votes & Proceedings, and committee documents.
//
// Live site reconnaissance (2026-09-09):
//   - https://www.parliament.cm/            → HTTP 200
//   - https://www.parliament.cm/bills       → HTTP 200 (Bills listing)
//
// The Parliament of Cameroon is UNICAMERAL — there is only one House, the
...
// Adapter adapts parliament.cm to the platform's
...
type Adapter struct {
	client    HTTPClient
	userAgent string
	// baseURL is the canonical Parliament of Cameroon root.
	baseURL string
...
type BillCandidate struct {
	...
	Title string
	// House is always "National Assembly of Cameroon" (Cameroon is unicameral).
	House string
...
// NewAdapter constructs a parliament.cm adapter. If client is nil a
...
func NewAdapter(client HTTPClient, userAgent string) *Adapter {
	...
	return &Adapter{
		client:    client,
		userAgent: userAgent,
		baseURL:   "https://www.parliament.cm",
		billsURL:  "https://www.parliament.cm/bills",
	}
}

// CountryCode returns "CM".
func (a *Adapter) CountryCode() string { return "CM" }

// Supports reports whether this adapter can handle the given URL. The
// Parliament adapter handles URLs from parliament.cm.
	...
	return []SourceDefinition{
		{
			...
			Country:        "CM",
			Institution:    "Parliament of Cameroon — Bills",
			Authority:      "primary",
			URL:            a.billsURL,
			Adapter:        "cameroon.parliament",
...
func (a *Adapter) DiscoverBills(ctx context.Context) ([]BillCandidate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cameroon.parliament.DiscoverBills: %w", err)
	}
	cands := ParseBillsListing(string(body))
	now := time.Now().UTC()
	for i := range cands {
		cands[i].House = "National Assembly of Cameroon"
		if cands[i].SourceID == "" {
...
func (a *Adapter) billCandidateToSourceItem(b BillCandidate) contracts.SourceItem {
	meta := map[string]string{
		"house":       b.House,
		"institution": "Parliament of Cameroon",
	}
	if b.BillNumber != "" {
...
func (a *Adapter) Fetch(ctx context.Context, item contracts.SourceItem) (*contracts.RawDocument, error) {
	if item.URL == "" {
		return nil, fmt.Errorf("cameroon.parliament.Fetch: empty URL")
	}
...
func (a *Adapter) Parse(ctx context.Context, doc contracts.RawDocument) ([]contracts.ExtractedRecord, error) {
	...
	if doc.MimeType == "application/pdf" || strings.HasSuffix(strings.ToLower(doc.URL), ".pdf") {
		return []contracts.ExtractedRecord{{
			...
			ExtractorName: "cameroon.parliament.PDFPlaceholder",
			Confidence:    0.1,
		}}, nil
	}
...
// structure data; callers should use the top-level CameroonAdapter.
func (a *Adapter) GetLegislativeStructure(ctx context.Context) (*contracts.LegislativeStructure, error) {
	_ = ctx
	return nil, fmt.Errorf("cameroon.parliament: GetLegislativeStructure not implemented on the source adapter; use the top-level CameroonAdapter")
}
...
func (a *Adapter) GetStages(ctx context.Context) ([]contracts.StageDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("cameroon.parliament: GetStages not implemented on the source adapter; use the top-level CameroonAdapter")
}
...
func (a *Adapter) GetTerminology(ctx context.Context) ([]contracts.TermDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("cameroon.parliament: GetTerminology not implemented on the source adapter; use the top-level CameroonAdapter")
}

// NormalizeSourceItem returns a minimal SourceItem so the contract surface
// compiles. The top-level CameroonAdapter owns its own normalizer.
...
func (a *Adapter) fetchURL(ctx context.Context, rawURL string) ([]byte, error) {
	if rawURL == "" {
		return nil, fmt.Errorf("cameroon.parliament.fetchURL: empty URL")
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("cameroon.parliament.fetchURL: HTTP %d for %s", resp.StatusCode, rawURL)
	}
	return io.ReadAll(resp.Body)
}

// makeSourceID derives a stable, deterministic SourceID for a Cameroon
...
// parliament.cm site. Production callers MUST NOT use this method.
