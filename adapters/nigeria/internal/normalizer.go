...
// NormalizeSourceItem maps a raw key/value record harvested from a Nigerian
// parliamentary source into the platform's contracts.SourceItem shape.
//
// The raw map's keys are the names used on nass.gov.ng (the National Assembly
// of Nigeria portal). By centralising the mapping here, the ingestion service
// never has to learn Nigerian field names.
//
...
func NormalizeSourceItem(raw map[string]any) (contracts.SourceItem, error) {
	if raw == nil {
		return contracts.SourceItem{}, errors.New("nigeria normalizer: nil raw map")
	}
	...
	title := getStr("title")
	if title == "" {
		// Nigeria's tracker sometimes uses "bill_title" instead.
	...
	if len(missing) > 0 {
		return contracts.SourceItem{}, fmt.Errorf("nigeria normalizer: missing required field(s): %s", strings.Join(missing, ", "))
	}
...
// mapSourceType maps Nigerian source-specific type strings to the platform's
// SourceItemType enum.
...
// mapHouse maps the Nigerian house string ("House of Representatives",
// "Senate", or their codes "HOR"/"SEN") into the adapter's canonical house code.
func mapHouse(s string) string {
	switch strings.ToLower(strings.TrimSpace(s)) {
	case "house of representatives", "houseofrepresentatives", "hor", "house":
		return HouseCodeHouseOfReps
	case "senate", "sen":
...
// timeParseFlexible tries the most common date layouts used by Nigerian
...
func timeParseFlexible(s string) (time.Time, error) {
	...
	return time.Time{}, fmt.Errorf("nigeria normalizer: cannot parse time %q", s)
}
