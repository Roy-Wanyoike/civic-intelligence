// Package parliament adapts the Parliament of Burkina Faso source
// (assemblee.bf). It implements discovery + fetch + parse for Bills,
// Hansard, Order Papers, Votes & Proceedings, and committee documents.
//
// Live site reconnaissance (2026-09-09):
//   - https://www.assemblee.bf/            → HTTP 200
//   - https://www.assemblee.bf/bills       → HTTP 200 (Bills listing)
//
// The Parliament of Burkina Faso is UNICAMERAL — there is only one House, the
...
// Adapter adapts assemblee.bf to the platform's
...
type Adapter struct {
	client    HTTPClient
	userAgent string
	// baseURL is the canonical Parliament of Burkina Faso root.
	baseURL string
...
type BillCandidate struct {
	...
	Title string
	// House is always "National Assembly of Burkina Faso" (Burkina Faso is unicameral).
	House string
...
// NewAdapter constructs a assemblee.bf adapter. If client is nil a
...
func NewAdapter(client HTTPClient, userAgent string) *Adapter {
	...
	return &Adapter{
		...
		billsURL:  "https://www.assemblee.bf/bills",
	}
}

// CountryCode returns "BF".
func (a *Adapter) CountryCode() string { return "BF" }

// Supports reports whether this adapter can handle the given URL. The
// Parliament adapter handles URLs from assemblee.bf.
	...
	return []SourceDefinition{
		{
			...
			Country:        "BF",
			Institution:    "Parliament of Burkina Faso — Bills",
			Authority:      "primary",
			URL:            a.billsURL,
			Adapter:        "burkina_faso.parliament",
...
func (a *Adapter) DiscoverBills(ctx context.Context) ([]BillCandidate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("burkina_faso.parliament.DiscoverBills: %w", err)
	}
	cands := ParseBillsListing(string(body))
	now := time.Now().UTC()
	for i := range cands {
		cands[i].House = "National Assembly of Burkina Faso"
		if cands[i].SourceID == "" {
...
func (a *Adapter) billCandidateToSourceItem(b BillCandidate) contracts.SourceItem {
	meta := map[string]string{
		"house":       b.House,
		"institution": "Parliament of Burkina Faso",
	}
	if b.BillNumber != "" {
...
func (a *Adapter) Fetch(ctx context.Context, item contracts.SourceItem) (*contracts.RawDocument, error) {
	if item.URL == "" {
		return nil, fmt.Errorf("burkina_faso.parliament.Fetch: empty URL")
	}
...
func (a *Adapter) Parse(ctx context.Context, doc contracts.RawDocument) ([]contracts.ExtractedRecord, error) {
	...
	if doc.MimeType == "application/pdf" || strings.HasSuffix(strings.ToLower(doc.URL), ".pdf") {
		return []contracts.ExtractedRecord{{
			...
			ExtractorName: "burkina_faso.parliament.PDFPlaceholder",
			Confidence:    0.1,
		}}, nil
	}
...
// structure data; callers should use the top-level BurkinaFasoAdapter.
func (a *Adapter) GetLegislativeStructure(ctx context.Context) (*contracts.LegislativeStructure, error) {
	_ = ctx
	return nil, fmt.Errorf("burkina_faso.parliament: GetLegislativeStructure not implemented on the source adapter; use the top-level BurkinaFasoAdapter")
}
...
func (a *Adapter) GetStages(ctx context.Context) ([]contracts.StageDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("burkina_faso.parliament: GetStages not implemented on the source adapter; use the top-level BurkinaFasoAdapter")
}
...
func (a *Adapter) GetTerminology(ctx context.Context) ([]contracts.TermDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("burkina_faso.parliament: GetTerminology not implemented on the source adapter; use the top-level BurkinaFasoAdapter")
}

// NormalizeSourceItem returns a minimal SourceItem so the contract surface
// compiles. The top-level BurkinaFasoAdapter owns its own normalizer.
...
func (a *Adapter) fetchURL(ctx context.Context, rawURL string) ([]byte, error) {
	if rawURL == "" {
		return nil, fmt.Errorf("burkina_faso.parliament.fetchURL: empty URL")
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("burkina_faso.parliament.fetchURL: HTTP %d for %s", resp.StatusCode, rawURL)
	}
	return io.ReadAll(resp.Body)
}

// makeSourceID derives a stable, deterministic SourceID for a Burkina Faso
...
// assemblee.bf site. Production callers MUST NOT use this method.
