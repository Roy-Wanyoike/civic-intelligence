// Package parliament — HTML parser for the Parliament of Mozambique Bills
// listing page (parlamento.gov.mz/bills).
//
// The Parliament of Mozambique publishes Bills using a card-per-bill layout that
...
// stageMapping maps the lowercased substrings the Parliament of Mozambique uses on
// its Bills pages to the canonical stage codes from internal/mozambique_data.go.
...
// MapStageText maps a raw stage string published by parlamento.gov.mz to the
// canonical Mozambique stage code. Returns "" when no mapping is known. The
...
// ParseBillsListing parses the HTML of a parlamento.gov.mz Bills listing
...
// ParseBillDetail extracts Bill metadata from a parlamento.gov.mz Bill detail
...
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	...
	if len(bills) == 0 {
		...
		if title == "" {
			return nil, fmt.Errorf("mozambique.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "mozambique.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		Kind:          "bill",
		Title:         b.Title,
		House:         "National Assembly of Mozambique",
		Stage:         MapStageText(b.Stage),
		Sponsor:       b.Sponsor,
		PublishedAt:   parseMozambiqueDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "mozambique.parliament.HTMLParser",
...
// parseMozambiqueDate tries the common date layouts used by parlamento.gov.mz.
// Returns the zero time if nothing parseable was provided.
func parseMozambiqueDate(s string) time.Time {
