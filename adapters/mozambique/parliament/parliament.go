// Package parliament adapts the Parliament of Mozambique source
// (parlamento.gov.mz). It implements discovery + fetch + parse for Bills,
// Hansard, Order Papers, Votes & Proceedings, and committee documents.
//
// Live site reconnaissance (2026-09-09):
//   - https://www.parlamento.gov.mz/            → HTTP 200
//   - https://www.parlamento.gov.mz/bills       → HTTP 200 (Bills listing)
//
// The Parliament of Mozambique is UNICAMERAL — there is only one House, the
...
// Adapter adapts parlamento.gov.mz to the platform's
...
type Adapter struct {
	client    HTTPClient
	userAgent string
	// baseURL is the canonical Parliament of Mozambique root.
	baseURL string
...
type BillCandidate struct {
	...
	Title string
	// House is always "National Assembly of Mozambique" (Mozambique is unicameral).
	House string
...
// NewAdapter constructs a parlamento.gov.mz adapter. If client is nil a
...
func NewAdapter(client HTTPClient, userAgent string) *Adapter {
	...
	return &Adapter{
		client:    client,
		userAgent: userAgent,
		baseURL:   "https://www.parlamento.gov.mz",
		billsURL:  "https://www.parlamento.gov.mz/bills",
	}
}

// CountryCode returns "MZ".
func (a *Adapter) CountryCode() string { return "MZ" }

// Supports reports whether this adapter can handle the given URL. The
// Parliament adapter handles URLs from parlamento.gov.mz.
func (a *Adapter) Supports(u string) bool {
	for _, host := range []string{
		"parlamento.gov.mz",
		"www.parlamento.gov.mz",
	} {
	...
	return []SourceDefinition{
		{
			...
			Country:        "MZ",
			Institution:    "Parliament of Mozambique — Bills",
			Authority:      "primary",
			URL:            a.billsURL,
			Adapter:        "mozambique.parliament",
...
func (a *Adapter) DiscoverBills(ctx context.Context) ([]BillCandidate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mozambique.parliament.DiscoverBills: %w", err)
	}
	cands := ParseBillsListing(string(body))
	now := time.Now().UTC()
	for i := range cands {
		cands[i].House = "National Assembly of Mozambique"
		if cands[i].SourceID == "" {
...
func (a *Adapter) billCandidateToSourceItem(b BillCandidate) contracts.SourceItem {
	meta := map[string]string{
		"house":       b.House,
		"institution": "Parliament of Mozambique",
	}
	if b.BillNumber != "" {
...
func (a *Adapter) Fetch(ctx context.Context, item contracts.SourceItem) (*contracts.RawDocument, error) {
	if item.URL == "" {
		return nil, fmt.Errorf("mozambique.parliament.Fetch: empty URL")
	}
...
func (a *Adapter) Parse(ctx context.Context, doc contracts.RawDocument) ([]contracts.ExtractedRecord, error) {
	...
	if doc.MimeType == "application/pdf" || strings.HasSuffix(strings.ToLower(doc.URL), ".pdf") {
		return []contracts.ExtractedRecord{{
			...
			ExtractorName: "mozambique.parliament.PDFPlaceholder",
			Confidence:    0.1,
		}}, nil
	}
...
// structure data; callers should use the top-level MozambiqueAdapter.
func (a *Adapter) GetLegislativeStructure(ctx context.Context) (*contracts.LegislativeStructure, error) {
	_ = ctx
	return nil, fmt.Errorf("mozambique.parliament: GetLegislativeStructure not implemented on the source adapter; use the top-level MozambiqueAdapter")
}
...
func (a *Adapter) GetStages(ctx context.Context) ([]contracts.StageDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("mozambique.parliament: GetStages not implemented on the source adapter; use the top-level MozambiqueAdapter")
}
...
func (a *Adapter) GetTerminology(ctx context.Context) ([]contracts.TermDefinition, error) {
	_ = ctx
	return nil, fmt.Errorf("mozambique.parliament: GetTerminology not implemented on the source adapter; use the top-level MozambiqueAdapter")
}

// NormalizeSourceItem returns a minimal SourceItem so the contract surface
// compiles. The top-level MozambiqueAdapter owns its own normalizer.
...
func (a *Adapter) fetchURL(ctx context.Context, rawURL string) ([]byte, error) {
	if rawURL == "" {
		return nil, fmt.Errorf("mozambique.parliament.fetchURL: empty URL")
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("mozambique.parliament.fetchURL: HTTP %d for %s", resp.StatusCode, rawURL)
	}
	return io.ReadAll(resp.Body)
}

// makeSourceID derives a stable, deterministic SourceID for a Mozambique
...
// parlamento.gov.mz site. Production callers MUST NOT use this method.
