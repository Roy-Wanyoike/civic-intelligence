// Package parliament — HTML parser for the Parliament of Niger Bills
// listing page (assemblee.ne/bills).
//
// The Parliament of Niger publishes Bills using a card-per-bill layout that
...
// stageMapping maps the lowercased substrings the Parliament of Niger uses on
// its Bills pages to the canonical stage codes from internal/niger_data.go.
...
// MapStageText maps a raw stage string published by assemblee.ne to the
// canonical Niger stage code. Returns "" when no mapping is known. The
...
// ParseBillsListing parses the HTML of a assemblee.ne Bills listing
...
// ParseBillDetail extracts Bill metadata from a assemblee.ne Bill detail
...
func ParseBillDetail(pageHTML, sourceURL string) ([]contracts.ExtractedRecord, error) {
	...
	if len(bills) == 0 {
		...
		if title == "" {
			return nil, fmt.Errorf("niger.parliament.ParseBillDetail: no Bill title found in %s", sourceURL)
		}
		rec := contracts.ExtractedRecord{
			...
			Confidence:    0.55,
			ExtractorName: "niger.parliament.HTMLParser",
		}
		if pdfURL != "" {
	...
	rec := contracts.ExtractedRecord{
		Kind:          "bill",
		Title:         b.Title,
		House:         "National Assembly of Niger",
		Stage:         MapStageText(b.Stage),
		Sponsor:       b.Sponsor,
		PublishedAt:   parseNigerDate(b.Date),
		SourceURL:     sourceURL,
		RetrievedAt:   time.Now().UTC(),
		Confidence:    0.85,
		ExtractorName: "niger.parliament.HTMLParser",
...
// parseNigerDate tries the common date layouts used by assemblee.ne.
// Returns the zero time if nothing parseable was provided.
func parseNigerDate(s string) time.Time {
